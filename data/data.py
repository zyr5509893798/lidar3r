...
class DUST3RSplattingDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data, coverage, resolution, alpha=0.85, beta=0.85):  # 移除num_epochs_per_epoch参数
        ...
        for seq_idx, sequence in enumerate(self.data.sequences):
            seq_length = len(self.data.color_paths[sequence])
            # 每个序列可生成的连续三帧数量 = 总帧数 - 2
            for start_frame in range(seq_length - 2):
                self.samples.append((seq_idx, start_frame))

    def __getitem__(self, idx):
        seq_idx, start_frame = self.samples[idx]
        sequence = self.data.sequences[seq_idx]
        camera_id = 0  # 固定使用相机0

        # 连续三帧: [start_frame, start_frame+1, start_frame+2]
        context_views = [start_frame, start_frame + 1]
        target_views = [start_frame + 2]

        views = {"context": [], "target": [], "scene": sequence}

        # 获取上下文帧 (连续的前两帧)
        for view_idx in context_views:
            view = self.data.get_view(sequence, view_idx, camera_id, self.resolution)
    ...
    def __len__(self):
        return len(self.samples)  # 总样本数 = 所有序列的(帧数-2)之和
...
#         first_context_view = random.randint(0, len(self.data.color_paths[sequence]) - 3) # 随便选图1,去掉最后两帧。
...
#         target_view = first_context_view + 2
#         return [first_context_view, second_context_view], [target_view], camera_id
